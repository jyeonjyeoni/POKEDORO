...
/** Synchronous SHA-256 used to keep PLACE-V1 working without Web Crypto. */
export function sha256(input:Uint8Array):Uint8Array {
  ...
  for(let offset=0;offset<paddedLength;offset+=64){
    ...
    for(let i=16;i<64;i++){const s0=rotateRight(words[i-15],7)^rotateRight(words[i-15],18)^(words[i-15]>>>3),s1=rotateRight(words[i-2],17)^rotateRight(words[i-2],19)^(words[i-2]>>>10);words[i]=(words[i-16]+s0+words[i-7]+s1)>>>0}
    let [a,b,c,d,e,f,g,h]=hash;
    for(let i=0;i<64;i++){const sum1=rotateRight(e,6)^rotateRight(e,11)^rotateRight(e,25),choose=(e&f)^(~e&g),temporary1=(h+sum1+choose+ROUND_CONSTANTS[i]+words[i])>>>0,sum0=rotateRight(a,2)^rotateRight(a,13)^rotateRight(a,22),majority=(a&b)^(a&c)^(b&c),temporary2=(sum0+majority)>>>0;h=g;g=f;f=e;e=(d+temporary1)>>>0;d=c;c=b;b=a;a=(temporary1+temporary2)>>>0}
